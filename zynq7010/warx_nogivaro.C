...
static uint16x8_t input[block], temp[block];
static uint16x8_t roundconstant;
static uint16x8_t allzero = vdupq_n_u16(0x0000);
static uint16x8_t xmm[8][8];
static uint16x8_t row[8];

/*************** GF(2^16) multiplication (replaces Givaro) ***************/
/*static u16 gf_mul(u16 a, u16 b) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        bb >>= 1;
        aa <<= 1;
        if (aa & 0x10000) aa ^= 0x1002B; // x^16 + x^5 + x^3 + x + 1
    }
    return (u16)(p & 0xFFFF);
}*/
static inline u16 gf_mul(u16 a, u16 b){
    if (a==0 || b==0) return 0;
    return gf_exp[gf_log[a] + gf_log[b]];
}

uint16x8_t mul(u16 operand, uint16x8_t x) {
    u16 tmp[8];
    vst1q_u16(tmp,x);
    for (int k = 0; k < 8; k++) {
        tmp[k] = gf_mul(operand, tmp[k]);
    }
...
int main(int argc, char** argv)
{
    ...
    printf("average CPU time for decrypting %d byte message on Zynq-7010: %.2f microseconds\n", 16*block, avetime*1e6);
    printf("average throughput for decrypting %d byte message on Zynq-7010: %.2f MB/s\n", 16*block, ((double)(16*block))/(double)(avetime*1024*1024));
    printf("average CPU cycles for decrypting %d byte message on Zynq-7010: %.2f\n", 16*block, avetime*666e6);
    printf("average CPB for decrypting %d byte message on Zynq-7010: %.2f\n", 16*block, avetime*666e6/(double)(16*block));
